...
ARCHIVO_CORTE = "TIEMPO-PRODUCCION.xlsx"
ARCHIVO_PROYECTOS = "proyectos.xlsx"
HTML_SALIDA = "produccion-estatus.html"
...
df_corte["FECHA"] = pd.to_datetime(df_corte["FECHA"], errors="coerce")
...
df_corte["DATETIME"] = pd.to_datetime(
    df_corte["FECHA"].dt.strftime("%Y-%m-%d") + " " + df_corte["HORA"],
    format="%Y-%m-%d %I:%M %p",
    errors="coerce"
)
...
<h1>Dashboard Producción TUNKERS de Mexico</h1>
...
for fecha in sorted(dias.keys(), reverse=True):
    ...
    for r in registros:
        ...
        <tr>
            ...
            <td>{r['fin'].strftime('%H:%M')}</td>
            <td>{segundos_a_hm(r['duracion'])}</td>
        </tr>
        """
